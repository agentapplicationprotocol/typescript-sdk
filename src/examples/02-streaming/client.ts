 ...
 *   - chunk: iterate text_delta events and write each piece as it arrives
...
async function main() {
  // chunk mode — receive and print each word as it streams in
  console.log("--- chunk mode ---");
  ...
  for await (const event of msgEvents) {
    ...
    if (event.event === "message_stop") console.log(`Stop: ${event.stopReason}`);
  }
}
